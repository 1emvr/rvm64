...
namespace superv::debug {
	...
	void print_decode(win_process* proc, vmcs_t* vmcs) {
		...
		while (offset < bytes_read) {
			...
			if (word == 0) {
				std::printf("%s 0x%016llx:  00 00 00 00 \n", (addr == vmcs.pc) ? ">" : " ", (unsigned long long)addr);
				offset += chunk;
				continue;
			}
			...
			if (count == 0) {
				std::printf("%s 0x%016llx:  %02x %02x %02x %02x   (undecodable)\n", (addr == vmcs.pc) ? ">" : " ", (unsigned long long)addr, buffer[offset+0], buffer[offset+1], buffer[offset+2], buffer[offset+3]);
				offset += chunk;
			} else {
				const cs_insn &ci = insn[0];
				std::printf("%s 0x%016llx:  ", (ci.address == vmcs.pc) ? ">" : " ", (unsigned long long)ci.address); 
